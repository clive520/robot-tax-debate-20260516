...
2. 分配正方、反方與裁判。
3. 收集必要背景資料與來源。
4. 透過 Chrome 開啟各 AI 網頁，由對應 AI 實際產出正方、反方與裁判內容。
5. 將各 AI 回覆複製回專案，整理成完整辯論與裁判評分。
6. 整理文字稿與網站資料。
7. 寫入 Supabase 資料庫。
8. 產生網站逐段語音。
9. 產生 Podcast 音訊。
10. 產生 YouTube 影片與 SRT 字幕。
11. 準備 YouTube 與 Spotify 上架資料。
12. 上架或排程 YouTube / Spotify。
13. 回填媒體網址到資料庫。
14. 建置、預覽、部署網站。
15. 檢查線上頁面與互動功能。
16. 登錄工作日誌。
...
本專案的核心原則是：正方、反方與裁判的文字，必須由被分配到該角色的 AI 實際產出。Codex 不得單方面代寫其他 AI 的正式辯論內容。

Codex 的任務是：

1. 整理題目、背景資料、回合規則與提示詞。
2. 使用 Chrome 開啟對應 AI 服務網頁。
3. 在使用者已登入帳號的情況下，將提示詞交給該 AI。
4. 取得該 AI 的回覆後，複製回專案。
5. 將回覆整理成 Markdown、資料庫段落、Podcast 腳本與網站內容。
6. 只做格式整理、錯字修正與必要的標點清理，不改寫對方立場或論證核心。

若 AI 網頁需要登入、驗證、付費確認或權限授權，Codex 應停下來請使用者操作，不得繞過登入或用其他模型冒名產出。

### 資料收集

在交給 AI 辯手前，Codex 需先收集必要背景資料。若題目涉及最新數據、法律、公共政策、醫療、金融或重大時事，必須優先使用官方資料或可信來源。
...
3. 重要背景事實。
4. 來源連結。
5. 哪些是事實資料，哪些是可供辯論的價值判斷。

這份資料要一併交給正方、反方與裁判，避免不同 AI 使用不同背景事實。

### Chrome 操作順序

每場辯論依下列順序進行：

1. 開啟正方 AI 網頁，送出「正方申論」提示詞。
2. 複製正方申論回專案。
3. 開啟反方 AI 網頁，提供題目、背景資料與正方申論，送出「反方申論」提示詞。
4. 複製反方申論回專案。
5. 回到正方 AI，提供反方申論，送出「正方駁論」提示詞。
6. 複製正方駁論回專案。
7. 回到反方 AI，提供正方駁論，送出「反方駁論」提示詞。
8. 複製反方駁論回專案。
9. 請反方 AI 產出「反方結辯」。
10. 複製反方結辯回專案。
11. 請正方 AI 閱讀反方結辯後產出「正方結辯」。
12. 複製正方結辯回專案。
13. 開啟裁判 AI 網頁，提供完整六段辯論、背景資料與評分表，請裁判評分。
14. 複製裁判評分、總分、勝方與理由回專案。
...
正方與反方不可看到裁判評分提示詞。裁判必須看到完整正反方內容。
...
整理到 `debate.md` 時，可以移除網頁 UI 中的無關文字，但不得改變該 AI 的實質論點。若需要大幅修稿，必須先告知使用者，或回到該 AI 重新要求修正。
